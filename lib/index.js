...
function readableReport(array) {
  const headerString =
    'As of today, the following students are missing journal entries:\n\n\n\n';

  const reportString = array.reduce((acc, cur) => {
    const name = Object.keys(cur);
    const { expect } = cur[name];
    const { missing } = cur[name];
    const newString = `${name} is missing ${missing} journal ${
      missing > 1 ? 'entries' : 'entry'
    } out of a required ${expect}.\n\n`;
